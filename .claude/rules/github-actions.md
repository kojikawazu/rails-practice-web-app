  ...
  changes:                      # 変更範囲を判定する
    ...
    steps:
      ...
      - uses: dorny/paths-filter@v3
        id: filter
        with:
          # predicate-quantifier: every ＝「すべてのパターンに一致したファイルだけを対象にする」。
          # 既定の some（いずれかに一致）のままだと、否定パターン単体が
          # 「除外対象以外のすべて」に一致してしまい、除外として機能しない。
...
- **除外リストは `predicate-quantifier: every` とセットで使う**。否定パターンだけを並べても、既定の quantifier では意図した除外にならない（設定ミスは「常に全ジョブが動く」方向に倒れるため気づきにくい）。
- **分類の期待値はテストか、少なくとも表として固定する**。代表パス（アプリのコード・spec・各 README・docs・rules・workflow・ビルド設定・未知の新規ディレクトリ）ごとに、どのジョブへ流れるかを検証できる状態にする。
...
- パスフィルタが**除外リスト方式**になっているか（対象リスト方式なら、新しいディレクトリが黙って対象外になる）。
- **どのジョブにも流れない変更**が生まれていないか（＝何も検証されずにマージできる経路）。
