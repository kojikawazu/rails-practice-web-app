...
| HTTP Verb Confusion | 1 つのアクションが複数の HTTP メソッドで到達できるとき、メソッド判定の取りこぼしで意図しない分岐に入る問題。Rails は **HEAD を GET ルートへ配送する**が `request.get?` は HEAD で false になるため、確認画面では**書き込み（POST）を明示判定**して GET・HEAD を読み取り側へ倒している。Brakeman が検出する。 | [`projects_controller.rb#confirm_new`](../rails-task-fullstack-web-app/app/controllers/projects_controller.rb)、[`tasks_controller.rb#confirm`](../rails-task-fullstack-web-app/app/controllers/tasks_controller.rb)。 |
...
| CSP（Content Security Policy） | ブラウザに「どこから読んだ資源なら実行してよいか」を宣言する HTTP ヘッダー。XSS を**入力検証・出力エスケープで取りこぼした場合の最後の防御**にする。インライン JS を禁止すると `onclick` 属性が動かなくなるため、Stimulus のアクションへ移す必要がある。 | [`content_security_policy.rb`](../rails-task-fullstack-web-app/config/initializers/content_security_policy.rb)、[`row_link_controller.js`](../rails-task-fullstack-web-app/app/javascript/controllers/row_link_controller.js)。 |
| nonce | インライン script を「アプリが意図して置いたもの」と示す使い捨ての乱数。CSP の `script-src` に `'nonce-...'` として載せ、同じ値を script タグにも付ける。**推測されると意味を失う**ためレスポンスごとに変える。 | [`csp_meta_tag`](../rails-task-fullstack-web-app/app/views/layouts/application.html.erb)、importmap のインライン script に自動付与。 |
...
| Active Storage | ファイル添付を扱う Rails の標準機能。ファイル実体は `blob`、レコードとの結び付きは `attachment` として別テーブルで管理する。**両者の id は独立して採番される別物**で、`task.images` が列挙するのは attachment（＝`image.id` は attachment の id）。削除指定を blob の id で行うと、選んでいない画像を消しかねない。 | Task の `has_many_attached :images`＝[`task.rb`](../rails-task-fullstack-web-app/app/models/task.rb)、[`task_image_service.rb#purge`](../rails-task-fullstack-web-app/app/services/task_image_service.rb)。 |
