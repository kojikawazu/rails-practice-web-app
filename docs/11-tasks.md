...
- [x] タスクにプレビュー URL（`preview_url`）を追加。確認画面で sandbox iframe プレビュー。http/https のみ許可（スキーム検証＋sandbox の多層防御で XSS/トップナビ乗っ取りを抑止）（PR #17）
...
| Day 4 | ✅ 完了（PR #4 マージ済み） |
| Day 5 | ✅ 完了（PR #5 マージ済み） |
| 追加対応（機能・テスト・CI） | ✅ 完了（PR #8〜#17 マージ済み） |
